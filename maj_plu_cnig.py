...
class MajPluCnigPlugin:
    ...
    def _load_outputs_to_group(self, output_paths):
        ...
        for path in output_paths:
            ...
            if not layer.isValid():
                raise ValueError(
                    f"Le fichier a été créé mais n'a pas pu être ajouté au panneau des couches : {normalized_path}"
                )
    ...
    def process_all(
        ...
        if not doc_source:
            raise ValueError(
                "DOC_URBA est obligatoire : sélectionnez une couche du projet ou un fichier DBF."
            )

        doc_com_source = self._resolve_source(
            doc_urba_com_layer_id, doc_urba_com_path
        )
        if not doc_com_source:
            raise ValueError(
                "DOC_URBA_COM est obligatoire : sélectionnez une couche du projet ou un fichier DBF."
            )
        ...
        doc_com_output = self._process_doc_urba_com(
            doc_com_source, output_folder, date_appro, new_idurba
        )
        outputs = [doc_output, doc_com_output]

        zone_source = self._resolve_source(
            zone_urba_layer_id, zone_urba_path
        )
        if zone_source:
            outputs.append(
                self._process_zone_urba(
                    zone_source,
                    output_folder,
                    date_appro,
                    new_idurba,
        ...
        prescription_source = self._resolve_source(
            prescription_surf_layer_id,
            prescription_surf_path,
        )
        if prescription_source:
            outputs.append(
                self._process_prescription_surf(
                    prescription_source,
                    output_folder,
                    date_appro,
                    new_idurba,
        ...
        prescription_lin_source = self._resolve_source(
            prescription_lin_layer_id,
            prescription_lin_path,
        )
        if prescription_lin_source:
            outputs.append(
                self._process_prescription_lin(
                    prescription_lin_source,
                    output_folder,
                    date_appro,
                    new_idurba,
                    nomreg,
        ...
        prescription_pct_source = self._resolve_source(
            prescription_pct_layer_id,
            prescription_pct_path,
        )
        if prescription_pct_source:
            outputs.append(
                self._process_prescription_pct(
                    prescription_pct_source,
                    output_folder,
                    date_appro,
                    new_idurba,
                    nomreg,
        ...
        info_source = self._resolve_source(
            info_surf_layer_id, info_surf_path
        )
        if info_source:
            outputs.append(
                self._process_info_surf(
                    info_source,
                    output_folder,
                    date_appro,
                    new_idurba,
        ...
        info_lin_source = self._resolve_source(
            info_lin_layer_id, info_lin_path
        )
        if info_lin_source:
            outputs.append(
                self._process_info_lin(
                    info_lin_source,
                    output_folder,
                    date_appro,
                    new_idurba,
    ...
    def _resolve_source(self, layer_id, browsed_path):
        ...
        if layer_id:
            ...
            if layer:
                ...
                raise ValueError(
                    f"La couche sélectionnée '{layer.name()}' ne pointe pas vers un fichier local exploitable."
                )

        return (
            os.path.abspath(os.path.normpath(browsed_path.strip()))
            if browsed_path
            else ""
        )
    ...
    def _process_doc_urba(
        ...
        match = re.match(
            r"^(?P<codecom>[^_]+)_DOC_URBA_(?P<olddate>\d{8})\.dbf$",
            filename,
            re.IGNORECASE,
        )
        ...
        if not match:
            raise ValueError(
                "DOC_URBA doit respecter le format codecommune_DOC_URBA_AAAAMMJJ.dbf"
            )
        ...
        output_path = os.path.abspath(
            os.path.normpath(
                os.path.join(
                    output_folder,
                    filename.replace(old_date, date_appro),
                )
            )
        )
        ...
        latest_feature = max(
            features,
            key=lambda feat: str(feat["DATAPPRO"] or ""),
        )

        next_nomproc = self._compute_next_nomproc(
            features,
            procedure_code,
        )

        new_idurba = self._build_plu_idurba_from_doc_filename(
            filename,
            date_appro,
        )
        ...
        try:
            ...
            if not layer.addFeature(new_feature):
                raise ValueError(
                    "Impossible d'ajouter la nouvelle ligne DOC_URBA."
                )
    ...
    def _ask_nomrapp_value(self, code_commune, date_appro):
        ...
        default_value = (
            f"{code_commune}_rapport_{date_appro}.pdf"
        )
        ...
        if not ok:
            raise ValueError(
                "Traitement annulé : aucun nom de fichier NOMRAPP n'a été validé."
            )
        ...
        if not custom_value:
            raise ValueError(
                "Le nom du fichier du rapport de présentation ne peut pas être vide."
            )

        return custom_value

    def _process_doc_urba_com(
        self,
        source_path,
        output_folder,
        date_appro,
        ...
        match = re.match(
            r"^(?P<codecom>[^_]+)_DOC_URBA_COM_(?P<olddate>\d{8})\.dbf$",
            filename,
            re.IGNORECASE,
        )
        ...
        if not match:
            raise ValueError(
                "DOC_URBA_COM doit respecter le format codecommune_DOC_URBA_COM_AAAAMMJJ.dbf"
            )
    ...
    def _process_zone_urba(
        ...
        output_path = self._dated_output_path(
            source_path,
            output_folder,
            date_appro,
            "ZONE_URBA",
    ...
    def _process_prescription_surf(
        self,
        source_path,
        output_folder,
        date_appro,
        ...
        return self._process_prescription_layer(
            ...
            expected_name="PRESCRIPTION_SURF",
            layer_name="PRESCRIPTION_SURF",
        )

    def _process_prescription_lin(
        self,
        source_path,
        output_folder,
        date_appro,
        ...
        return self._process_prescription_layer(
            ...
            nomreg=nomreg,
            code_commune=code_commune,
            expected_name="PRESCRIPTION_LIN",
            layer_name="PRESCRIPTION_LIN",
        )

    def _process_prescription_pct(
        self,
        source_path,
        output_folder,
        date_appro,
        ...
        return self._process_prescription_layer(
            ...
            nomreg=nomreg,
            code_commune=code_commune,
            expected_name="PRESCRIPTION_PCT",
            layer_name="PRESCRIPTION_PCT",
        )

    def _process_prescription_layer(
        self,
        source_path,
        output_folder,
        date_appro,
        ...
        try:
            for feat in layer.getFeatures():
                ...
                if self._is_reglement_nomfic(
                    ...
                    doc_type = (
                        self._extract_document_type_from_nomfic(
                            nomfic,
                            code_commune,
                        )
                    )
                    ...
                    if doc_type not in update_choices:
                        update_choices[doc_type] = (
                            self._ask_update_nomfic_date(
                                layer_name,
                                doc_type,
                                nomfic,
                            )
    ...
    def _process_info_surf(
        ...
        # NOMFIC ne doit pas être modifié pour INFO_SURF :
        # les fichiers annexes ne changent pas.
        self._require_fields(
            layer,
            ["IDURBA"],
            "INFO_SURF",
    ...
    def _is_reglement_nomfic(
        ...
        return value.startswith(
            f"{code_commune.lower()}_reglement"
        )

    def _extract_document_type_from_nomfic(
        self,
        nomfic,
        code_commune,
    ):
        """
        Extrait TypeDocument depuis
        codecommune_TypeDocument_AAAAMMJJ.pdf.

        Exemple :
        45188_orientations_amenagement_20221213.pdf
        ...
        pattern = (
            rf"^{re.escape(code_commune)}_"
            rf"(?P<doctype>.+?)_\d{{8}}\.pdf$"
        )
    ...
    def _copy_dataset_family(
        self,
        input_path,
        output_path,
    ):
        if not input_path or not os.path.exists(
            input_path
        ):
            raise ValueError(
        ...
        input_root, _ = os.path.splitext(
            input_path
        )

        output_root, _ = os.path.splitext(
            output_path
        )

        if input_root.lower() == output_root.lower():
            raise ValueError(
                "Le fichier de sortie est identique au fichier source. "
                "Choisissez un dossier de sortie différent ou une nouvelle date d'approbation."
            )

        if self._dataset_family_exists(
            output_path
        ):
            if not self._confirm_replace(
                ...
                raise ValueError(
                    f"Traitement annulé : le fichier existe déjà et ne sera pas remplacé : {output_path}"
                )

            self._delete_dataset_family(
                output_path
            )
    ...
    def _dated_output_path(
        ...
        if not match:
            raise ValueError(
                f"{expected_name} doit respecter le format "
                f"codecommune_{expected_name}_AAAAMMJJ{expected_ext}"
            )
    ...
    def _open_layer(
        ...
        if not layer.isValid():
            raise ValueError(
                f"Impossible d'ouvrir {name} avec QGIS/OGR : {path}"
            )

        return layer

    def _ensure_text_field(
        self,
        layer,
        field_name,
        length,
        ...
        if not layer.dataProvider().addAttributes(
            ...
            raise ValueError(
                f"Impossible de créer le champ {field_name}."
            )
    ...
    def _require_fields(
        ...
        missing = [
            ...
            if layer.fields().indexFromName(f)
            == -1
        ]

        if missing:
            raise ValueError(
                f"Champs obligatoires absents dans {layer_name} : "
                + ", ".join(missing)
            )
    ...
    def _start_edit(
        ...
        if not layer.startEditing():
            raise ValueError(
                f"Impossible de passer {layer.name()} en mode édition."
            )
    ...
    def _change(
        ...
        idx = layer.fields().indexFromName(
            field_name
        )
        ...
        if not layer.changeAttributeValue(
            ...
            raise ValueError(
                f"Impossible de modifier {field_name}."
            )
    ...
    def _compute_next_nomproc(
        self,
        features,
        procedure_code,
    ):
        pattern = re.compile(
            rf"^{re.escape(procedure_code)}(\d+)$",
            re.IGNORECASE,
        )
        ...
        return (
            f"{procedure_code}{max_number + 1}"
        )

    def _build_plu_idurba_from_doc_filename(
        self,
        filename,
        date_appro,
    ):
        """
        Construit l'IDURBA à partir du code commune
        du fichier DOC_URBA, en produisant toujours :

        codecommune_PLU_AAAAMMJJ.

        Exemple :
        78683_DOC_URBA_20220922.dbf
        -> 78683_PLU_20260207.
        """

        code_commune = filename.split(
            "_",
            1,
        )[0]

        return (
            f"{code_commune}_PLU_{date_appro}"
        )

    def _replace_last_date_without_ext(
        self,
        filename,
        date_appro,
    ):
